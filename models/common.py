...
def make_bn_layer(plane):
    return nn.BatchNorm2d(plane)


def convbn(in_planes, out_planes, kernel_size=3, stride=1, padding=1, dilation=1, bias=True, no_relu=False):
    layers = []
    layers.append(
        nn.Conv2d(
    ...
    layers.append(make_bn_layer(out_planes))
    if not no_relu:
...
def predict_occ(in_planes, num_classes=192, bias=True):
    return nn.Sequential(nn.Conv2d(in_planes, num_classes, kernel_size=3, stride=1, padding=1, bias=bias), nn.Sigmoid())
...
class Hourglass(nn.Module):
    def __init__(self, in_planes, do_flow=False, no_output=False ):
...
class PPM(nn.Module):
    def __init__(
        ...
        for scale in pool_scales:
            ...
            self.ppm_conv.append(
                nn.Sequential(
                    ...
                    make_bn_layer( ppm_inter_conv_planes),
        ...
        self.ppm_last_conv = convbn(
            encoder_planes[-1] + len(pool_scales) * 128, self.ppm_last_conv_planes, bias=False 
        )
    ...
    def forward(self, conv5):
        ...
        for pool_scale, pool_conv in zip(self.ppm_pooling, self.ppm_conv):
            ppm_out.append(
                pool_conv(
                    F.upsample(pool_scale(conv5), (input_size[2], input_size[3]), mode="bilinear", align_corners=False)
                )
            )
...
def make_refinement_module(module_type, in_planes, do_flow=False, no_output=False): 
    ...
    if module_type == "none":
        ...
        return Hourglass(in_planes, do_flow, no_output)
    else:
