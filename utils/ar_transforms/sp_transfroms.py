...
from utils.ar_transforms.interpolation import Interp2
from utils.ar_transforms.interpolation import Meshgrid
import numpy as np
...
class RandomAffineFlow(nn.Module):
    ...
    def find_invalid(self, width, height, thetas):
        ...
        xq, yq = denormalize_coords(xq, yq, width=width, height=height)
        #
        invalid = ((xq < 0) | (yq < 0) | (xq >= width) | (yq >= height)).sum(dim=1, keepdim=True) > 0
    ...
    def transform_flow(self, flow, theta1, theta2):
        ...
        x0, y0 = self.inverse_transform_coords(width=width, height=height, thetas=theta1)

        x1, y1 = self.inverse_transform_coords(width=width, height=height, thetas=theta2, offset_x=u, offset_y=v)
    ...
    def forward(self, tgt_img, ref_imgs, org_output):
        imgs = [tgt_img] + ref_imgs
        flows_f = [flow_ref[0].detach() for flow_ref in org_output["flow_bwd"]]
        # masks_f = [occ_ref[0].detach() for occ_ref in org_output["occ"]]

        batch_size, _, height, width = tgt_img.shape
        ...
        flows_f = [self.transform_flow(flows_f[i], theta_list[0], theta_list[1 + i]) for i in range(len(flows_f))]

        # masks_f = [self.transform_image(mask, theta) for mask, theta in zip(masks_f, theta_list[1:])]
        ...
        return imgs[0], imgs[1:], {"flow": flows_f}
